...
extern xQueueHandle queue_tecla;
...
void taskCDC(void * a)
{
	...
	while (1) {
		...
		if (prompt && xQueueReceive( queue_tecla, &key, portMAX_DELAY ) == pdTRUE) {
				int i;
				for (i=0; i<6; i++) {
					ciaaWriteOutput(i, key & (1 << i));
				}
				vcom_write((uint8_t *)&key, sizeof(char));
		}
